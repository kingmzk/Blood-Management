...
import template as abus

con = sq.connect("Report.db")
c = con.cursor()
...
class main_page(tk.Frame):
	def __init__(self):
		...
		self.pack()
		self.master.title("Blood Management System")
		self.text1 = tk.Text(self, height=5, width=65,bg='green')


		self.text1.grid(row=4, column=7)

		self.button_1 = tk.Button(self, text='Register your Info',bg='Blue', command = self.register)
		self.button_1.grid(row=7, column=7)
		self.button_2 = tk.Button(self, text='Enter Blood group and City Name',bg='Blue', command = self.get_info)
		self.button_2.grid(row=9, column=7)
		self.button_3 = tk.Button(self, text='Choose Your Blood Group',bg='Blue',command = abus.about_us_display)
	...
	def register(self):
		...
		self.name = tk.Label(self.win, text='Enter your Name:')
		...
		self.phone = tk.Label(self.win, text='Contact No Please.:')
		self.address = tk.Label(self.win, text='Your  Address:')
	...
	def print_data(self):
		...
		if(count==0):
			...
			tk.Label(self.C, text='''Sorry!!
              Looks like a rare blood group in your city!
               Please Try searching in any other city!!''').grid(row=0, column=0)
